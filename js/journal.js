...
window.ZenJournal = (function () {
  ...
  const LETTER_SYSTEM_PROMPT =
    ...
    '像认识他们很久、看着他们一次次来的旧朋友。\n\n' +
    '写信的原则：\n' +
    '1. 动笔前先想清楚这封信要说的一两件事，信是围着事写的，不是围着他们的记录清单写的\n' +
    '2. 引用他们日记里的原话时，必须用「」把原话框起来，让读者一眼看出哪些是他们说的。除框原话外，全文不得出现任何其他引号\n' +
    '3. 不要复述或点评他们的每条记录。最多挑一到两处真正值得回应的细节，像老朋友聊天时随口提起，其余一概不提\n' +
    '4. 段与段之间顺着同一件事或同一个心思往下说，不要一句日记配一句感想地跳来跳去\n' +
    '5. 信里要有一件你这边最近发生的小事，比如天气的变化、来过的动物、某个傍晚的光线、附近人的动静。这件事是你编的，但要具体、贴合当地，像分享近况一样放进信里，给它一个自然的位置\n' +
    '6. 口语化，句子有长有短，允许一点点絮叨\n' +
    '7. 全文禁止破折号、书名号、括号补充、波浪线，禁止比喻堆砌，禁止排比\n' +
    '8. 结尾禁止总结升华，禁止愿你如何如何的句式，禁止出现喧嚣、内心、平静、治愈这类词\n' +
    '9. 自然分三到四段，总长 300 到 500 字\n' +
    '10. 落款只写这个地方的名字，日期写今天的\n' +
    '11. 直接输出信的正文，不要任何解释或标题';
  ...
  async function generateLetter() {
    ...
    var userPrompt = persona + '\n\n' +
      ...
      JSON.stringify(llmData, null, 2) + '\n\n' +
      (hasText
        ? '这些记录只是给你参考的素材，不是清单，不必都用到。只挑一到两处真正值得回应的写，引用原话时用「」框起来，其余记录不要在信里逐条出现。'
        : '他们几乎没有留下文字，就从时间习惯和场景选择这些细节里挑一两处写，像老朋友记得对方的习惯那样。') +
      '\n现在写下这封信。';
